...
function OnboardingWizard({ t, onDone, email, setEmail }) {
  ...
  return (
    <section className="card onboarding-card">
      ...
      {step === 4 && (<><h2>{t.activity}</h2><label>{t.activityLevel}<select value={form.activityLevel} onChange={(e) => update('activityLevel', e.target.value)}><option value="low">{t.low}</option><option value="moderate">{t.moderate}</option><option value="high">{t.high}</option></select></label><p>{t.readyToStart}</p></>)}
...
function DashboardPage({ t, profile }) {
  return <section className="card"><h1>{t.dashboard}</h1><p>{t.dailyTarget}: <strong>{profile?.dailyCaloriesTarget || 0} kcal</strong></p><p>{t.subtitle}</p></section>
}

function ProfilePage({ t, profile }) {
  if (!profile) return <section className="card">{t.loading}</section>
  return <section className="card"><h2>{t.profile}</h2><div className="grid two"><div><strong>{t.name}:</strong> {profile.name}</div><div><strong>{t.email}:</strong> {profile.email}</div><div><strong>{t.country}:</strong> {profile.country}</div><div><strong>{t.goal}:</strong> {profile.goal}</div><div><strong>{t.currentWeight}:</strong> {profile.currentWeightKg}</div><div><strong>{t.targetWeight}:</strong> {profile.targetWeightKg}</div></div></section>
}
      ...
      {open && (
        <div className="chat-overlay" onClick={() => setOpen(false)}>
          <div className="chat-panel" onClick={(e) => e.stopPropagation()}>
            <div className="chat-header"><strong>{t.aiChat}</strong><button className="icon-btn" onClick={() => setOpen(false)}>✕</button></div>
            <div className="chat-box">
              {messages.length === 0 && <p>{t.aiPlaceholder}</p>}
              {messages.map((m, i) => <div key={i} className={`bubble ${m.role}`}>{m.content}</div>)}
            </div>
            <div className="chat-input"><input value={text} onChange={(e) => setText(e.target.value)} placeholder={t.chatInputPlaceholder} /><button onClick={send} disabled={sending}>{sending ? '...' : t.send}</button></div>
          </div>
        </div>
      )}
    </>
...
function AdminDashboardPage({ t }) {
  ...
  useEffect(() => {
    Promise.all([apiGet('/api/admin/metrics'), apiGet('/api/admin/users'), apiGet('/api/admin/chats')]).then(([m, u, c]) => {
      setMetrics(m)
      setUsers(u)
      setChats(c)
    }).catch(() => setMetrics({ totalUsers: 0, totalChats: 0, totalMessages: 0, newUsers7d: 0 }))
  }, [])

  return (
    <section className="card">
      <h2>{t.adminDashboard}</h2>
      <div className="metrics-grid"><div className="metric"><span>{t.totalUsers}</span><strong>{metrics?.totalUsers ?? '-'}</strong></div><div className="metric"><span>{t.totalChats}</span><strong>{metrics?.totalChats ?? '-'}</strong></div><div className="metric"><span>{t.totalMessages}</span><strong>{metrics?.totalMessages ?? '-'}</strong></div><div className="metric"><span>{t.newUsers7d}</span><strong>{metrics?.newUsers7d ?? '-'}</strong></div></div>
      <h3>{t.usersList}</h3><div className="table-wrap"><table><thead><tr><th>{t.name}</th><th>{t.email}</th><th>{t.goal}</th><th>{t.country}</th></tr></thead><tbody>{users.map((u) => <tr key={u._id}><td>{u.name}</td><td>{u.email}</td><td>{u.goal}</td><td>{u.country}</td></tr>)}</tbody></table></div>
      <h3>{t.chatsList}</h3><div className="table-wrap"><table><thead><tr><th>{t.chatTitle}</th><th>{t.messagesCount}</th><th>{t.lastUpdate}</th></tr></thead><tbody>{chats.map((c) => <tr key={c._id}><td>{c.title}</td><td>{c.messages?.length || 0}</td><td>{new Date(c.updatedAt).toLocaleString()}</td></tr>)}</tbody></table></div>
    </section>
  )
}
